...
\chapter{Analysis}
This chapter identifies the requirements, needed to transform the application into a stand-alone front-end, serving the micro-services back-end.\\
...
\begin{usecase}
	...
	\addfield{Summary:}{Upload multiple files to the Websuite, with the same metadata, to save time for files that are handled alike.}
	\additemizedfield{Actors:}{
		\item User
	}
	\additemizedfield{Preconditions:}{
...
\begin{usecase}
	...
	\additemizedfield{Primary Scenario:}{
		\item The user checks the status of multiple imported files by searching for them. As a result he sees \textit{Finished} or \textit{Failed}.
	}
\end{usecase}
